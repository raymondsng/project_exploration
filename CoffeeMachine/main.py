...
if __name__ == '__main__':
    while operation:
        ...
        if status == "user":
            user_input = input("What would you like to drink today? (expresso/latte/cappuccino): ")
            ...
            if amount >= MENU[user_input]['cost']:
                ...
                print(f"Sorry, the money you've inserted is not sufficient. Refunding ${amount}...")
                continue
            deduct_resources(MENU, user_input)
            resources["money"] += amount - (amount - MENU[user_input]['cost'])
            print(f"Here is your {user_input} ☕ Enjoy!")
        elif status == "admin":
            admin_input = input("Task Manager: What action would you like to perform (report/restock/off): ")
            if admin_input == "off":
